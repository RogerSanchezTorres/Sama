...
<body>
    ...
    <h2>{{ $subsubcategory->nombre }}</h2><br>
                ...
                <ul class="categorias-list">
                    ...
                    <li>
                        <a href="{{ route('products.showProductsByCategory', ['categorySlug' => $relatedCategory->slug]) }}"
                            @if ($subsubcategory->subcategory->category->id === $relatedCategory->id) class="selected" @endif>
                            {{ $relatedCategory->nombre }}
                        </a>
                        @if ($subsubcategory->subcategory->category->id === $relatedCategory->id)
                        ...
                        <ul class="subcategorias-list">
                            ...
                            <li>
                                <a href="{{ route('products.showProductsBySubcategory', ['subcategorySlug' => $subcat->slug]) }}"
                                    @if ($subsubcategory->subcategory->id === $subcat->id) class="selected" @endif>
                                    {{ $subcat->nombre }}
                                </a>
                                @if ($subsubcategory->subcategory->id === $subcat->id && $subcat->subsubcategories->count() > 0)
